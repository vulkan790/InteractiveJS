...
const isActive = (path: string): boolean => router.currentRoute.value.path === path
...
const handleTasksClick = (event: MouseEvent): void => {
...
const handleProfileClick = (event: MouseEvent): void => {
    if (!isAuthenticated.value) 
    {
        event.preventDefault()
        router.push('/login')
